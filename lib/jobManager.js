    ...
    if (options.kawpow_validator == "kawpowd") {
      ...
      _this.daemon.cmd('getkawpowhash', [ header_hash, miner_given_mixhash, miner_given_nonce, job.rpcData.height, job.target_hex ], function (results) {
        if (!results || !results[0] || !results[0].response) {
          // RPC temporarily unavailable — accept share optimistically, block submission
          // will naturally fail if hash is invalid. Don't punish miner for node hiccup.
          emitLog("getkawpowhash RPC unavailable, accepting share optimistically");
          _this.emit('share', {
