...
var JobCounter = function () {
  ...
  counter_buf.fill(0, 0, 24);
  counter_buf.writeUIntBE(counter & 0xFFFFFFFFFFFF, 26, 6);
...
var JobManager = module.exports = function JobManager(options) {
  ...
  // SOLO POOL: per-miner block templates keyed by miner address
  // Each miner mines a block whose coinbase pays to their own address
  this.minerTemplates = {};  // address -> blockTemplate
  this.minerJobLookup = {};  // jobId -> { template, address }
  ...
  // SOLO POOL: build a block template whose coinbase pays the given miner address
  // dev fee recipients are preserved from options.recipients
  this.getJobForMiner = function(minerAddress) {
    if (typeof _this.currentRpcData === 'undefined') return null;

    // Reuse the miner's existing template if it's still valid for the current
    // block. Regenerating would issue a new jobId and orphan the miner's
    // in-flight work ("job not found" on submit).
    var existing = _this.minerTemplates[minerAddress];
    if (existing &&
        existing.rpcData.previousblockhash === _this.currentRpcData.previousblockhash &&
        existing.rpcData.height === _this.currentRpcData.height) {
      return existing;
    }

    var tmpl;
    try {
      tmpl = new blockTemplate(
        jobCounter.next(),
        _this.currentRpcData,
        options.coin.reward,
        options.recipients,   // dev fee (2%) preserved
        minerAddress          // coinbase pays THIS miner, not the pool
      );
    } catch (e) {
      // Invalid miner address — fall back to pool address so mining continues,
      // but log loudly so the operator notices.
      emitErrorLog('Invalid miner address "' + minerAddress + '": ' + e +
                   ' — falling back to pool address');
      tmpl = new blockTemplate(
        jobCounter.next(),
        _this.currentRpcData,
        options.coin.reward,
    ...
    _this.minerTemplates[minerAddress] = tmpl;
    _this.minerJobLookup[tmpl.jobId] = { template: tmpl, address: minerAddress };
    _this.validJobs[tmpl.jobId] = tmpl;

    return tmpl;
  };

  this.updateCurrentJob = function (rpcData) {
    _this.currentRpcData = rpcData;
  ...
  this.processTemplate = function (rpcData) {
    ...
    // SOLO POOL: always keep the latest rpcData so getJobForMiner can build
    // a fresh coinbase for any miner address on demand
    _this.currentRpcData = rpcData;
    ...
    this.validJobs = {};
    // SOLO POOL: new block invalidates all per-miner templates; they get
    // rebuilt on demand when each miner requests work
    this.minerTemplates = {};
    this.minerJobLookup = {};
  ...
  this.processShare = function (miner_given_jobId, previousDifficulty, difficulty, miner_given_nonce, ipAddress, port, workerName, miner_given_header, miner_given_mixhash, extraNonce1, callback_parent) {
    ...
    if (options.kawpow_validator == "kawpowd") {
      ...
      // Skip getkawpowhash daemon validation — it floods node RPC queue
      // and causes node crashes. Accept share directly, only submit if
      // hash meets block target (checked via submitblock which handles rejection).
      var headerBigNum = bignum(header_hash, 16);
      if (job.target.ge(headerBigNum)) {
        var blockHex = job.serializeBlock(
          Buffer.from(header_hash, 'hex'),
          Buffer.from(miner_given_mixhash.replace('0x',''), 'hex'),
          miner_given_nonce
        ).toString('hex');
        var blockHash = header_hash;
        _this.emit('share', {
          job: miner_given_jobId, ip: ipAddress, port: port, worker: workerName,
          height: job.rpcData.height, blockReward: job.rpcData.coinbasevalue,
          difficulty: difficulty, shareDiff: difficulty,
          blockDiff: job.difficulty * shareMultiplier, blockDiffActual: job.difficulty,
          blockHash: blockHash, blockHashInvalid: undefined
        }, blockHex);
        callback_parent({result: true, error: null, blockHash: blockHash});
      } else {
        _this.emit('share', {
          ...
          difficulty: difficulty, shareDiff: difficulty,
          blockDiff: job.difficulty * shareMultiplier, blockDiffActual: job.difficulty,
          blockHash: undefined, blockHashInvalid: undefined
        }, undefined);
        callback_parent({result: true, error: null, blockHash: null});
      }
    }
  };s = require('events');
var crypto = require('crypto');
    ...
    if (options.kawpow_validator == "kawpowd") {
      ...
      _this.daemon.cmd('getkawpowhash', [ header_hash, miner_given_mixhash, miner_given_nonce, job.rpcData.height, job.target_hex ], function (results) {
        if (!results || !results[0] || !results[0].response) {
          // getkawpowhash RPC unavailable — accept share for miner stats but
          // do NOT submit block to network (avoids flooding node with bad blocks)
          emitLog("getkawpowhash RPC unavailable, share counted but not submitted");
          _this.emit('share', {
            job: miner_given_jobId, ip: ipAddress, port: port, worker: workerName,
            height: job.rpcData.height, blockReward: job.rpcData.coinbasevalue,
            difficulty: difficulty, shareDiff: undefined,
            blockDiff: job.difficulty * shareMultiplier, blockDiffActual: job.difficulty,
            blockHash: undefined, blockHashInvalid: undefined
          }, null);
          callback_parent({result: true, error: null, blockHash: null});
