...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function SetupJobManager() {
        ...
        // SOLO POOL: extract miner payout address from worker name.
        // Worker name format: <address>.<rigname> (e.g. "co8z5...rig1")
        // The address before the dot is where this miner's block reward goes.
        _this.getMinerAddress = function(client) {
            var wn = client.workerName || '';
            var addr = wn.split('.')[0];
            return addr;
        };

        // SOLO POOL: build and send a job to this specific client whose
        // coinbase pays the client's own address.
        _this.sendSoloJobToClient = function(client) {
            var addr = _this.getMinerAddress(client);
            if (!addr) {
                // Not authorized yet; fall back to current job (will be replaced post-auth)
                if (_this.jobManager.currentJob)
                    client.sendMiningJob(_this.jobManager.currentJob.getJobParams());
                return;
            }
            var tmpl = _this.jobManager.getJobForMiner(addr);
            if (tmpl) {
                client.sendMiningJob(tmpl.getJobParams());
            } else if (_this.jobManager.currentJob) {
                client.sendMiningJob(_this.jobManager.currentJob.getJobParams());
            }
        };
        ...
        _this.jobManager.on('newBlock', function (blockTemplate) {
            ...
            if (_this.stratumServer) {
                // SOLO POOL: each connected miner gets a fresh job whose
                // coinbase pays their own address (not a shared pool job).
                var clients = _this.stratumServer.getStratumClients();
                for (var clientId in clients) {
                    _this.sendSoloJobToClient(clients[clientId]);
                }
            }
        }).on('updatedBlock', function (blockTemplate) {
            //Check if stratumServer has been initialized yet
            if (_this.stratumServer) {
                // SOLO POOL: only push refreshed jobs to miners whose template
                // actually changed (new transactions). getJobForMiner reuses the
                // existing template for the same block, so we compare jobIds and
                // only send when it's genuinely new — avoids 5s job spam.
                var clients = _this.stratumServer.getStratumClients();
                for (var clientId in clients) {
                    var c = clients[clientId];
                    var addr = _this.getMinerAddress(c);
                    if (!addr) continue;
                    var prev = _this.jobManager.minerTemplates[addr];
                    var prevId = prev ? prev.jobId : null;
                    var tmpl = _this.jobManager.getJobForMiner(addr);
                    if (tmpl && tmpl.jobId !== prevId) {
                        c.sendMiningJob(tmpl.getJobParams());
                    }
                }
            }
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('started', function () {
            ...
            // SOLO POOL: send each connected client their own per-miner job
            var clients = _this.stratumServer.getStratumClients();
            for (var clientId in clients) {
                _this.sendSoloJobToClient(clients[clientId]);
            }
            ...
            client.on('difficultyChanged', function (diff) {
                ...
                // SOLO POOL: send a job whose coinbase pays this miner.
                // At subscription the worker may not be authorized yet; if so,
                // the authorization handler will send the correct per-miner job.
                _this.sendSoloJobToClient(client);

            }).on('authorization', function (params) {
                // SOLO POOL: now that the worker is authorized and we know its
                // address, send it a job whose coinbase pays that address.
                _this.sendSoloJobToClient(client);
    ...
    function GetBlockTemplate(callback) {
        _this.daemon.cmd('getblocktemplate',
            ...
            function (result) {
                if (result.error) {
                    // FIXED: was using undefined 'logger', now using emitErrorLog
    ...
    this.attachMiners = function (miners) {
        ...
        // SOLO POOL: send each attached miner their own per-miner job
        var clients = _this.stratumServer.getStratumClients();
        for (var clientId in clients) {
            _this.sendSoloJobToClient(clients[clientId]);
        }
